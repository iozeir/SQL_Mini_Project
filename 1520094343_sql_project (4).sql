...
SELECT tab.court, CONCAT( tab.firstname,  ' ', tab.surname ) AS name
FROM (
...
GROUP BY tab.court, tab.firstname, tab.surname
ORDER BY name
...
SELECT tab.newname, SUM( tab.cost ) AS revenue
FROM (

SELECT Facilities.name AS newname, 
CASE WHEN Bookings.memid =0
THEN Facilities.guestcost * Bookings.slots
ELSE Facilities.membercost * Bookings.slots
END AS cost
...
group by  tab.newname
)tab1
where  tab1.revenue <1000
LIMIT 0 , 30
